...
export const makeSocket = (config: SocketConfig) => {
	...
	const onWhatsApp = async (...users: string[]) => {
		...
		for (const input of users) {
			...
			// LID: query directly by JID so onWhatsApp can return LID data together with jid.
			if (isLidUser(input)) {
				usyncQuery.withUser(new USyncUser().withId(input))
				continue
			}

			// PN / JID / plain phone number
			const decoded = jidDecode(input)
			const phoneUser = decoded?.user || input.replace('+', '').split('@')[0]?.split(':')[0]
		...
		return results.list.map(entry => ({
			jid: (entry.id as string) || null,
			lid: (entry.lid as string) || null,
	...
	return {
		...
		signalRepository,
		getLIDForPN: (pn: string) => signalRepository.lidMapping.getLIDForPN(pn),
		getLIDsForPNs: (pns: string[]) => signalRepository.lidMapping.getLIDsForPNs(pns),
		getPNForLID: (lid: string) => signalRepository.lidMapping.getPNForLID(lid),
		get user() {
